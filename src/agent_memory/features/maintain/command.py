...
class MaintCtx:
    ...
    apply_safe: bool
    no_llm: bool
    ollama_up: bool  # really "LLM capable": local Ollama OR cloud fallback
...
def _cloud_available() -> bool:
    """True when the cheap_llm cloud fallback is enabled and importable.

    Lets the audit loop gate on LLM *capability* (local OR cloud) instead of
    daemon liveness alone — otherwise the fallback path in ``_cloud_call``
    would be unreachable whenever Ollama is down, which is exactly when it
    exists for."""
    if os.environ.get("AGENT_MEMORY_CLOUD_FALLBACK", "1") == "0":
        return False
    import importlib.util

    return importlib.util.find_spec("cheap_llm") is not None
...
def maintain(
    ...
    cloud_up = not ollama_up and not disabled and _cloud_available()
    # Audit capability, not daemon liveness: the cloud fallback exists for the
    # daemon-down case, so gating _emit_audit on ollama_up alone would make the
    # fallback unreachable exactly when it is needed.
    ctx = MaintCtx(apply_safe=apply_safe, no_llm=no_llm, ollama_up=ollama_up or cloud_up)
    ctx.report = [
    ...
    if not ctx.ollama_up:
        ...
        ctx.report.extend(["> ⚠️ Ollama unavailable — audits run via cheap_llm cloud fallback.", ""])
