...
- **Optional (LLM):** local Ollama — `embeddinggemma` (768-d embeds), `batiai/gemma4-e4b:q4` (maintain/audit; shares `CODEQ_SUMMARY_MODEL` with codeq, round-6 winner), `cryptidbleh/gemma4-claude-opus-4.6:latest` (infra fallback). Degrades gracefully; `maintain` also falls back to the ecosystem `cheap_llm` cloud cascade when Ollama is down (env `AGENT_MEMORY_CLOUD_FALLBACK`, default on).
...
| `CODEQ_SUMMARY_MODEL` | `batiai/gemma4-e4b:q4` | Maintain/audit model (tracks codeq round-6 winner) |
